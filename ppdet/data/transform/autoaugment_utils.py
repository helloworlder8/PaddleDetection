...
def autocontrast(image):
    ...
    def scale_channel(image):
        ...
        def scale_values(img):
            ...
            img = np.clip(img, a_min=0, a_max=255.0)
...
def equalize(image):
    ...
    def scale_channel(img, c):
        """Scale the data in the channel to implement equalize."""
        img = img[:, :, c].astype(np.int32)
        # Compute the histogram of the image channel.
        histo, _ = np.histogram(img, range=[0, 255], bins=256)
        ...
        if step == 0:
            ...
            result = np.take(build_lut(histo, step), img)
