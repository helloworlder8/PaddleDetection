...
class BaseOperator(object): #给一个身份id
...
class Decode(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        elif sample['h'] != img.shape[0]:
            logger.warning(
                "The actual image height: {} is not equal to the "
                "height: {} in annotation, and update sample['h'] by actual "
                "image height.".format(img.shape[0], sample['h']))
            sample['h'] = img.shape[0]
        if 'w' not in sample:
            sample['w'] = img.shape[1]
        elif sample['w'] != img.shape[1]:
            logger.warning(
                "The actual image width: {} is not equal to the "
                "width: {} in annotation, and update sample['w'] by actual "
                "image width.".format(img.shape[1], sample['w']))
            sample['w'] = img.shape[1]

        sample['im_shape'] = np.array(img.shape[:2], dtype=np.float32)
...
class SniperDecodeCrop(BaseOperator):
    ...
    def __call__(self, sample, context=None):
        ...
        img = sample['image']
        data = np.frombuffer(img, dtype='uint8')
        img = cv2.imdecode(data, cv2.IMREAD_COLOR)  # BGR mode, but need RGB mode
        if 'keep_ori_im' in sample and sample['keep_ori_im']:
            sample['ori_image'] = img
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        ...
        img = img[max(y1, 0):min(y2, img.shape[0]), max(x1, 0):min(x2, img.shape[
            1]), :]
    ...
    def apply(self, sample, context=None):
        ...
        for idx in range(gt_bbox.shape[0]):
            ...
            if w < w_bbox and h < h_bbox:
                ...
                img[int(y1 + off_y1):int(y1 + off_y1 + h), int(x1 + off_x1):int(
                    x1 + off_x1 + w), :] = 0
        sample['image'] = img
        return sample
...
class NormalizeImage(BaseOperator):
    def __init__(self,
        ...
        if not (isinstance(self.mean, list) and isinstance(self.std, list) and isinstance(self.is_scale, bool) and
...
class AutoAugment(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        img, norm_gt_bbox = distort_image_with_autoaugment(img, norm_gt_bbox,
                                                          self.autoaug_type)
...
class Resize(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        if len(img.shape) == 3:
            img = self.apply_image(sample['image'], [im_scale_x, im_scale_y])
...
class MultiscaleTestResize(BaseOperator):
    ...
    def apply(self, sample, context=None):
        """ Resize the image numpy for multi-scale paddle.
        """
        samples = []
        resizer = Resize(
...
class RandomExpand(BaseOperator):
    ...
    def __init__(self, ratio=4., prob=0.5, fill_value=(127.5, 127.5, 127.5)):
        ...
        if not isinstance(fill_value, tuple):
            fill_value = tuple(fill_value) #转成元祖形式
        ...
        if prob > self.sampling_prob:  # anchor sampling
            ...
            while sampled_bbox:
                ...
                if self.avoid_no_bbox:
                    if len(crop_bbox) < 1:
                        continue
                img = crop_image_sampling(img, sample_bbox, image_width,
...
class Mixup(BaseOperator):
    ...
    def __call__(self, sample, context=None):
        ...
        if factor <= 0.0:
            return sample[1]
        img = self.apply_image(sample[0]['image'], sample[1]['image'], factor)
        result = copy.deepcopy(sample[0])
        result['image'] = img
        ...
        if 'gt_reid' in sample[0]:
            gt_ide1 = sample[0]['gt_reid']
            gt_ide2 = sample[1]['gt_reid']
            gt_reid = np.concatenate((gt_ide1, gt_ide2), axis=0)
            result['gt_reid'] = gt_reid
        return result
...
class PadBox(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        if 'gt_reid' in sample:
            pad_ide = np.zeros((num_max, ), dtype=np.int32)
            if gt_num > 0:
                pad_ide[:gt_num] = sample['gt_reid'][:gt_num, 0]
            sample['gt_reid'] = pad_ide
        return sample
...
class Pad(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        sample['image'] = self.apply_image(img, offsets, im_size, size)
...
class RandomShortSideResize(BaseOperator):
    ...
    def resize(self,
        ...
        target_size = self.get_size_with_aspect_ratio(img.shape[:2], target_size,
                                                      max_size)
        im_scale_y, im_scale_x = target_size[1] / img.shape[0], target_size[
            0] / img.shape[1]

        sample['image'] = cv2.resize(img, target_size, interpolation=interp)
        ...
        # apply polygon
        if 'gt_poly' in sample and len(sample['gt_poly']) > 0:
            sample['gt_poly'] = self.apply_segm(sample['gt_poly'], img.shape[:2],
                                                [im_scale_x, im_scale_y])
        # apply semantic
        if 'semantic' in sample and sample['semantic']:
            semantic = sample['semantic']
...
class RandomShift(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        canvas[coor_new[1]:coor_new[3], coor_new[0]:coor_new[2]] \
            = img[coor_old[1]:coor_old[3], coor_old[0]:coor_old[2]]
...
class StrongAugImage(BaseOperator):
    ...
    def apply(self, sample, context=None):
        img = sample
        img['image'] = sample['image'].astype('uint8')
        results = self.transforms(img)
...
class RandomGaussianBlur(BaseOperator):
    ...
    def apply(self, sample, context=None):
        if np.random.uniform(0, 1) < self.prob:
            ...
            img = cv2.GaussianBlur(sample['image'], (23, 23), sigma)
            sample['image'] = img
        return sample
    ...
    def apply(self, sample, context=None):
        if random.random() < self.prob:
            ...
            top, left, erase_h, erase_w, v = self._get_param(img, self.scale,
                                                             self.ratio, value)
            img = self._erase(img, top, left, erase_h, erase_w, v, self.inplace)
            sample['image'] = img
        return sample
