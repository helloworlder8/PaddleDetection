...
class COCODataSet(DetDataset):
    ...
    def parse_dataset(self):
        ...
        coco = COCO(anno_path) #直接调用coco的api
        img_ids = coco.getImgIds()
        img_ids.sort() #总共591张图片
        cat_ids = coco.getCatIds() #总共4类
        records = []
        empty_records = []
        ct = 0
        ...
        for img_id in img_ids:
            ...
            if not self.load_image_only:
                ins_anno_ids = coco.getAnnIds(
                    imgIds=[img_id], iscrowd=None if self.load_crowd else False) #scrowd=0 时 表示标注的物体是一个单独的、可独立检测的对象。例如，一只猫、一辆车等。  iscrowd=1 时 表示标注的是一群密集的对象，通常是难以逐一检测的。例如，集体行走的一群人、成堆的物品等。
                ...
                num_bbox = len(bboxes)
                if num_bbox <= 0 and not self.allow_empty: #图像不允许有空标签
                    continue
                elif num_bbox <= 0:
                    is_empty = True
                ...
                gt_track_id = -np.ones((num_bbox, 1), dtype=np.int32) #(5, 1)
                ...
                for k, v in gt_rec.items():
                    if k in self.data_fields:
                        coco_rec[k] = v #coco在多一些信息

                # TODO: remove load_semantic
                if self.load_semantic and 'semantic' in self.data_fields:
                    seg_path = os.path.join(self.dataset_dir, 'stuffthingmaps', 'train2017', im_fname[:-3] + 'png')
    ...
    def parse_dataset(self):
        ...
        for img_id in img_ids:
            ...
            for _ind in range(sub_img_num):
                img = slice_image_result.images[_ind]
                coco_rec = {
                    'image': img,
