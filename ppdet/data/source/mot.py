...
class MOTDataSet(DetDataset):
    ...
    def parse_dataset(self):
        ...
        self.seqs_tid_max = OrderedDict()
        self.seqs_tid_start_ind = OrderedDict()

        img_index = 0
        for seq_name in self.image_lists: #不同的数据集
            ...
            img_lists_path = os.path.join(image_lists_dir, seq_name)
            ...
            with open(img_lists_path, 'r') as file:
                ...
                self.seqs_img_start_ind[seq_name] = img_index #，每一个数据集中的第一个图片
        ...
        for seq_name, label_paths in self.seqs_label_paths.items(): #遍历每一个标注文件夹
            ...
            for label_path_i in label_paths:
                ...
                if tid_max > seq_tid_max:
                    seq_tid_max = tid_max
            self.seqs_tid_max[seq_name] = int(seq_tid_max + 1)

        seqs_tid_sum_dict = 0
        for i, (k, v) in enumerate(self.seqs_tid_max.items()):
            self.seqs_tid_start_ind[k] = seqs_tid_sum_dict
            seqs_tid_sum_dict += v

        self.seqs_tid_sum_dict = defaultdict(int)
        self.seqs_tid_sum_dict[0] = int(seqs_tid_sum_dict + 1) 
        self.num_imgs_each_data = [len(x) for x in self.seqs_img_paths.values()]
        self.total_imgs = sum(self.num_imgs_each_data)

        logger.info('MOT dataset summary: ')
        logger.info(self.seqs_tid_max)
        logger.info('Total images: {}'.format(self.total_imgs))
        logger.info('Image start index: {}'.format(self.seqs_img_start_ind))
        logger.info('Total trajectory: {}'.format(self.seqs_tid_sum_dict[0]))
        logger.info('Trajectory start index: {}'.format(self.seqs_tid_start_ind))
        ...
        for img_index in range(self.total_imgs):
            ...
            gt_tid = labels[:, 1:2].astype('int32')
            for i, _ in enumerate(gt_tid):
                if gt_tid[i] > -1:
                    gt_tid[i] += self.seqs_tid_start_ind[seq_name]
...
class MCMOTDataSet(DetDataset):
    ...
    def parse_dataset(self):
        ...
        img_index = 0
        for seq_name in self.image_lists:
            # check every data image list
            image_lists_dir = os.path.join(self.dataset_dir, 'image_lists')
            assert os.path.isdir(image_lists_dir), \
                "The {} is not a directory.".format(image_lists_dir)
        ...
        for seq_name, label_paths in self.seqs_label_paths.items():
            # using max_ids_dict rather than seq_tid_max
            ...
            # track id number
            self.seqs_tid_max[seq_name] = max_ids_dict

        last_idx_dict = defaultdict(int)
        for i, (k, v) in enumerate(self.seqs_tid_max.items()):  # each sub dataset
        ...
        for k, v in last_idx_dict.items():
            self.seqs_tid_sum_dict[k] = int(v)  # total ids of each category
        ...
        seqs_tid_sum_dict = sorted(
            self.seqs_tid_sum_dict.items(), key=lambda x: x[0])
        total_IDs_all_cats = 0
        for (k, v) in seqs_tid_sum_dict:
