...
def get_config_path(url):
    ...
    """
    url = parse_url(url)
    path = map_path(url, CONFIGS_HOME, url_path_depth=2)
...
def map_path(url, local_dir, url_path_depth=1):
    # parse path after download to decompress under local_dir
    assert url_path_depth > 0, "url_path_depth should be a positive integer"
    father_url = url
    for _ in range(url_path_depth):
        father_url = osp.dirname(father_url)
    file_name = osp.relpath(url, father_url)

    zip_formats = ['.zip', '.tar', '.gz']
    for zip_format in zip_formats:
        file_name = file_name.replace(zip_format, '')
    return osp.join(local_dir, file_name)


def get_path(url, local_dir, md5sum=None, check_exist=True):
    """ Download from given url to local_dir.
    if file or directory specified by url is exists under
    local_dir, return the path directly, otherwise download
    from url and decompress it, return the path.

    url (str): download url
    local_dir (str): root dir for downloading, it should be
                    WEIGHTS_HOME or DATASET_HOME
    md5sum (str): md5 sum of download package
    """
    # parse path after download to decompress under local_dir
    local_file = map_path(url, local_dir)
    ...
    for k, v in decompress_name_map.items():
        if local_file.find(k) >= 0:
            local_file = osp.join(osp.split(local_file)[0], v)

    if osp.exists(local_file) and check_exist:
        if not osp.isfile(local_file) or \
                _check_exist_file_md5(local_file, md5sum, url):
            logger.debug("Found {}".format(local_file))
            return local_file, True
        else:
            os.remove(local_file)

    local_file = _download_dist(url, local_dir, md5sum)

    # new weights format which postfix is 'pdparams' not
    # need to decompress
    if osp.splitext(local_file)[-1] not in ['.pdparams', '.yml', '.ttf']:
        _decompress_dist(local_file)
...
def _download(url, local_dir, md5sum=None):
    """
    ...
    """
    must_mkdirs(local_dir)

    file_name = osp.split(url)[-1]
    local_file = osp.join(local_dir, file_name)
    retry_cnt = 0

    while not (osp.exists(local_file) and _check_exist_file_md5(local_file, md5sum,
                                                              url)):
        ...
        logger.info("Downloading {} from {}".format(file_name, url))
        ...
        # tmp_local_file firstly, move tmp_local_file to local_file
        # after download finished
        tmp_local_file = local_file + "_tmp"
        total_size = req.headers.get('content-length')
        with open(tmp_local_file, 'wb') as f:
            if total_size:
                for chunk in tqdm.tqdm(req.iter_content(chunk_size=1024),total=(int(total_size) + 1023) // 1024,unit='KB'):
                    f.write(chunk) #持续写
            else:
                for chunk in req.iter_content(chunk_size=1024):
                    if chunk:
                        f.write(chunk)
        shutil.move(tmp_local_file, local_file)
    return local_file


def _download_dist(url, local_dir, md5sum=None):
    env = os.environ #分布式下载
    if 'PADDLE_TRAINERS_NUM' in env and 'PADDLE_TRAINER_ID' in env:
        ...
        if num_trainers <= 1:
            return _download(url, local_dir, md5sum)
        else:
            file_name = osp.split(url)[-1]
            local_file = osp.join(local_dir, file_name)
            lock_path = local_file + '.download.lock'
            ...
            if not osp.exists(local_file):
                ...
                if rank_id_curr_node == 0:
                    _download(url, local_dir, md5sum)
                    os.remove(lock_path)
                else:
...
def _md5check(local_file, md5sum=None):
    if md5sum is None:
        return True

    logger.debug("File {} md5 checking...".format(local_file))
    md5 = hashlib.md5()
    ...
    if calc_md5sum != md5sum:
        logger.warning("File {} md5 check failed, {}(calc) != "
                       "{}(base)".format(local_file, calc_md5sum, md5sum))
        return False
    return True


def _decompress(file_name):
    """
    Decompress for zip and tar file
    """
    logger.info("Decompressing {}...".format(file_name))

    # For protecting decompressing interupted,
    # decompress to fpath_tmp directory firstly, if decompress
    # successed, move decompress files to fpath and delete
    # fpath_tmp and remove download compress file.
    fpath = osp.split(file_name)[0]
    ...
    if file_name.find('tar') >= 0:
        ...
        raise TypeError("Unsupport compress file type {}".format(file_name))
...
def _decompress_dist(file_name):
    env = os.environ
    if 'PADDLE_TRAINERS_NUM' in env and 'PADDLE_TRAINER_ID' in env:
        trainer_id = int(env['PADDLE_TRAINER_ID'])
        num_trainers = int(env['PADDLE_TRAINERS_NUM'])
        if num_trainers <= 1:
            _decompress(file_name)
        else:
            lock_path = file_name + '.decompress.lock'
