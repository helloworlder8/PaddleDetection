...
def load_pretrain_weight(model, pretrain_weight, ARSL_eval=False):
    ...
    """ 权重路径 """
    if is_url(pretrain_weight): #可以是url可以是文件路径
        pretrain_weight = get_weights_path(pretrain_weight)
    clean_path = _strip_postfix(pretrain_weight)
    if not (os.path.isdir(clean_path) or os.path.isfile(clean_path) or
            os.path.exists(clean_path + '.pdparams')):
        raise ValueError("Model pretrain path `{}` does not exists. "
                         "If you don't want to load pretrain model, "
                         "please delete `pretrain_weights` field in "
                         "config file.".format(clean_path))
        
        
    """ 权重值加载 """   
    teacher_student_flag = False
    if not ARSL_eval: #有教师模型和学生模型
        if hasattr(model, 'modelTeacher') and hasattr(model, 'modelStudent'):
            print('Loading pretrain weights for Teacher-Student framework.')
            print(
            ...
            model_dict = model.state_dict()
        local_weights = clean_path + '.pdparams'
        param_state_dict = paddle.load(local_weights)
        ...
        if teacher_student_flag:
            ...
            model.set_dict(param_state_dict) #权重赋值
        logger.info('Finish loading model weights: {}'.format(local_weights))
