...
@register
class DETR(BaseArch): #整体网络结构
    __category__ = 'architecture' #说明这是一个完整的检测模型
    __inject__ = ['post_process', 'post_process_semi'] #说明这个模型中的这些处理使用其他的类进行处理的
    __shared__ = ['with_mask', 'exclude_post_process'] #这个模型的共享参数，说明这些参数全模型通用
