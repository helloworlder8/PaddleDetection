...
class FairMOT(BaseArch): #FairMOT模型架构
    ...
    def __init__(self,
        ...
        self.detector = detector #可以理解成一个平行的类
        self.reid = reid
    ...
    def _forward(self):
        ...
        det_outs = self.detector(self.inputs) #[2, 3, 608, 1088]
        neck_feat = det_outs['neck_feat'] #->[2, 64, 152, 272]
        if self.training:
            reid_loss = self.reid(neck_feat, self.inputs) #[2, 64, 152, 272]
