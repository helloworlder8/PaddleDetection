...
class BBoxPostProcess(object):
    ...
    def __call__(self, preds, bs_rois_and_num, im_shape, scale_factor):
        ...
        Args:
            preds (tuple): bbox_pred and cls_prob of bbox_head output.
            bs_rois_and_num (tuple): roi and rois_num of rpn_head output.
        ...
        Returns:
            ...
            bbox_num (Tensor): The number of prediction boxes of each batch with
                shape [1], and is N.
        """
        if self.nms is not None: #expand_bs_rois_and_num, pd_bbox_class_without_bg
            expand_bs_rois_and_num, pd_bbox_class_without_bg = self.decode(preds, bs_rois_and_num, im_shape, scale_factor)
            nms_prediction, nms_prediction_num, before_nms_indexes = self.nms(expand_bs_rois_and_num, pd_bbox_class_without_bg, self.num_classes)

        else:
            nms_prediction, nms_prediction_num = self.decode(preds, bs_rois_and_num, im_shape,
                                              scale_factor)
        ...
        if self.export_onnx:
            ...
            nms_prediction = paddle.concat([nms_prediction, fake_bboxes])
            nms_prediction_num = nms_prediction_num + 1

        if self.nms is not None:
            return nms_prediction, nms_prediction_num, before_nms_indexes
        else:
            return nms_prediction, nms_prediction_num

    def get_pred(self, bboxes, bbox_num, im_shape, scale_factor): #nms_prediction, nms_prediction_num, im_shape, scale_factor
        """
...
class JDEBBoxPostProcess(nn.Layer):
    ...
    def forward(self, preds, anchors):
        """
        Decode the bbox and do NMS for JDE model. 

        Args:
            preds (list): Bbox_pred and cls_prob of bbox_head output.
        ...
        """
        boxes_idx, yolo_boxes_scores = self.decode(preds, anchors)
...
class DETRPostProcess(object):
    ...
    def __call__(self, preds, im_shape, scale_factor, pad_shape):
        """
        Decode the bbox and mask.

        Args:
            preds (tuple): bbox_pred, cls_logit and masks of bbox_head output.
