...
class BBoxHead(nn.Layer):
    ...
    # bs_rois, bs_rois_num
    def forward(self, body_feats=None, bs_rois=None, bs_rois_num=None, inputs=None, cot=False):
        """body_feats, bs_rois, bs_rois_num, None
        body_feats (list[Tensor]): Feature maps from backbone
        bs_rois (list[Tensor]): RoIs generated from RPN module
        bs_rois_num (Tensor): The number of RoIs in each image
        inputs (dict{Tensor}): The ground-truth of image
        """
        if self.training: #rois, rois_num, targets bs_rois    bs_rois 先筛选
            bs_rois, bs_rois_num, targets = self.bbox_assigner(bs_rois, bs_rois_num, inputs)
            self.assigned_rois = (bs_rois, bs_rois_num)
            self.assigned_targets = targets

        rois_feat = self.roi_extractor(body_feats, bs_rois, bs_rois_num) #->[1024, 256, 7, 7] #7x7窗口，每个窗口256维度，总共1024个proposals
        shared_fc_feat = self.head(rois_feat) #->[1024, 1024] 推理是[2000, 1024]
        if self.with_pool:
            shared_fc_feat = F.adaptive_avg_pool2d(shared_fc_feat, output_size=1)
            shared_fc_feat = paddle.squeeze(shared_fc_feat, axis=[2, 3])

        if self.use_cot:
            pd_bbox_class = self.cot_bbox_score(shared_fc_feat)
            cot_scores = self.bbox_score(shared_fc_feat)
        else:
            pd_bbox_class = self.bbox_score(shared_fc_feat) #->[1024, 5]
        pd_bbox_deltas = self.bbox_delta(shared_fc_feat)
        ...
        if self.training:
            ...
            return loss, shared_fc_feat
        else:
            if cot:
                pred = self.get_prediction(cot_scores, pd_bbox_deltas)
            else:
                pred = self.get_prediction(pd_bbox_class, pd_bbox_deltas) #5个类别 4个anchor
    ...
    def get_loss(self,
        ...
        pd_bbox_deltas (Tensor): pd_bbox_deltas from bbox head outputs
        targets (list[List[Tensor]]): bbox targets containing tgt_bbox_class, tgt_bboxes and tgt_gt_labels
        bs_proposals_box (List[Tensor]): RoIs generated in each batch
        """
        ...
        tgt_bbox_class, tgt_bboxes, tgt_gt_labels = targets

        # bbox cls
        tgt_bbox_class = paddle.concat(tgt_bbox_class) if len(tgt_bbox_class) > 1 else tgt_bbox_class[0]
        valid_inds = paddle.nonzero(tgt_bbox_class >= 0).flatten()
        if valid_inds.shape[0] == 0:
            loss_bbox[cls_name] = paddle.zeros([1], dtype='float32')
        else:
            tgt_bbox_class = tgt_bbox_class.cast('int64')
            tgt_bbox_class.stop_gradient = True

            if not loss_normalize_pos:
                loss_bbox_cls = F.cross_entropy(
                    input=pd_bbox_class, label=tgt_bbox_class, reduction='mean')
            else:
                loss_bbox_cls = F.cross_entropy(
                    input=pd_bbox_class, label=tgt_bbox_class,
                    reduction='none').sum() / (tgt_bbox_class.shape[0] + 1e-7)

            loss_bbox[cls_name] = loss_bbox_cls

        # bbox reg

        cls_agnostic_bbox_reg = pd_bbox_deltas.shape[1] == 4

        fg_inds = paddle.nonzero(
            paddle.logical_and(tgt_bbox_class >= 0, tgt_bbox_class <
                               self.num_classes)).flatten()

        if fg_inds.numel() == 0:
            # loss_bbox[reg_name] = paddle.zeros([1], dtype='float32')
            loss_bbox[reg_name] = pd_bbox_class.mean() * 0. + pd_bbox_deltas.mean() * 0.
            return loss_bbox

        if cls_agnostic_bbox_reg:
            pd_bbox_deltas = paddle.gather(pd_bbox_deltas, fg_inds)
        else:
            fg_gt_classes = paddle.gather(tgt_bbox_class, fg_inds)
            ...
            pd_bbox_deltas = paddle.gather(pd_bbox_deltas, fg_inds)
            pd_bbox_deltas = paddle.gather_nd(pd_bbox_deltas, reg_inds).reshape([-1, 4])
        bs_proposals_box = paddle.concat(bs_proposals_box) if len(bs_proposals_box) > 1 else bs_proposals_box[0]
        tgt_bboxes = paddle.concat(tgt_bboxes) if len(
            tgt_bboxes) > 1 else tgt_bboxes[0]

        reg_target = bbox2delta(bs_proposals_box, tgt_bboxes, bbox_weight) #提取和真实的偏差
        reg_target = paddle.gather(reg_target, fg_inds)
        reg_target.stop_gradient = True

        if self.bbox_loss is not None:
            pd_bbox_deltas = self.bbox_transform(pd_bbox_deltas)
            reg_target = self.bbox_transform(reg_target)

            if not loss_normalize_pos:
                loss_bbox_reg = self.bbox_loss(
                    pd_bbox_deltas, reg_target).sum() / tgt_bbox_class.shape[0]
                loss_bbox_reg *= self.num_classes

            else:
                loss_bbox_reg = self.bbox_loss(
                    pd_bbox_deltas, reg_target).sum() / (tgt_bbox_class.shape[0] + 1e-7)

        else:
            loss_bbox_reg = paddle.abs(pd_bbox_deltas - reg_target).sum(
            ) / tgt_bbox_class.shape[0] #0124
    ...
    def get_prediction(self, pd_bbox_class, pd_bbox_deltas):
        pd_bbox_class = F.softmax(pd_bbox_class) #pd_bbox_class, pd_bbox_deltas
        return pd_bbox_deltas, pd_bbox_class
