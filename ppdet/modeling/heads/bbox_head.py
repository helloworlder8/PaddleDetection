...
class BBoxHead(nn.Layer):
    ...
    def forward(self, body_feats=None, rois=None, rois_num=None, inputs=None, cot=False):
        ...
        rois_feat = self.roi_extractor(body_feats, rois, rois_num) #->[1024, 256, 7, 7]
        bbox_feat = self.head(rois_feat) #->[1024, 1024]
        if self.with_pool:
        ...
        if self.use_cot:
            ...
            scores = self.bbox_score(feat) #->[1024, 5]
    ...
    def get_loss(self,
        ...
        targets (list[List[Tensor]]): bbox targets containing gt_score, gt_bboxes and tgt_gt_labels
        ...
        if valid_inds.shape[0] == 0:
            ...
            gt_score.stop_gradient = True

            if not loss_normalize_pos:
                loss_bbox_cls = F.cross_entropy(
                    input=scores, label=gt_score, reduction='mean')
            else:
                loss_bbox_cls = F.cross_entropy(
                    input=scores, label=gt_score,
                    reduction='none').sum() / (gt_score.shape[0] + 1e-7)
        ...
        fg_inds = paddle.nonzero(
            paddle.logical_and(gt_score >= 0, gt_score <
        ...
        if cls_agnostic_bbox_reg:
            ...
            fg_gt_classes = paddle.gather(gt_score, fg_inds)
        ...
        gt_bboxes = paddle.concat(gt_bboxes) if len(
            gt_bboxes) > 1 else gt_bboxes[0]

        reg_target = bbox2delta(rois, gt_bboxes, bbox_weight)
        ...
        if self.bbox_loss is not None:
            ...
            if not loss_normalize_pos:
                loss_bbox_reg = self.bbox_loss(
                    reg_delta, reg_target).sum() / gt_score.shape[0]
                loss_bbox_reg *= self.num_classes

            else:
                loss_bbox_reg = self.bbox_loss(
                    reg_delta, reg_target).sum() / (gt_score.shape[0] + 1e-7)
