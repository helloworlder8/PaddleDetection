...
class S2ANetHead(nn.Layer):
    ...
    def get_bboxes(self, head_outs):
        ...
        for i in range(batch):
            pred_scores_per_imgage = [t[i] for t in pred_scores_list]
            pred_bboxes_per_imgage = [t[i] for t in perd_bboxes_list]
            bbox_per_imgage, bbox_num_per_imgage = self.get_bboxes_single(
                pred_scores_per_imgage, pred_bboxes_per_imgage)
            bboxes.append(bbox_per_imgage)
            bbox_num.append(bbox_num_per_imgage)
    ...
    def get_loss(self, head_outs, inputs):
        ...
        for i in range(batch):
            ...
            anchors_per_imgage = np.concatenate(base_anchors_list)

            fam_cls_per_imgage = [t[i] for t in fam_cls_list]
            fam_reg_per_imgage = [t[i] for t in fam_reg_list]
            odm_cls_per_imgage = [t[i] for t in odm_cls_list]
            odm_reg_per_imgage = [t[i] for t in odm_reg_list]
            im_s2anet_head_out = (fam_cls_per_imgage, fam_reg_per_imgage,
                                  odm_cls_per_imgage, odm_reg_per_imgage,
                                  num_anchors_list)
            # FAM
            im_fam_target = self.anchor_assign(anchors_per_imgage, gt_bboxes,
                                               gt_labels, is_crowd)
            ...
            # ODM
            refine_anchors_per_imgage = [t[i] for t in refine_anchors_list]
            refine_anchors_per_imgage = paddle.concat(
                refine_anchors_per_imgage).numpy()
            im_odm_target = self.anchor_assign(refine_anchors_per_imgage,
