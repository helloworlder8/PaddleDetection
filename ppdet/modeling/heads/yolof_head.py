...
class YOLOFHead(nn.Layer):
    ...
    def get_loss(self, head_outs, targets):
        ...
        for cls_logit, bbox_pred, gt_bbox, gt_class, im_shape in zip(
            ...
            match_boxes_labels, pos_bbox_pred, pos_bbox_tar = self.bbox_assigner(
                bbox_pred, anchors, gt_bbox)
            pos_mask = (match_boxes_labels >= 0)
            neg_mask = (match_boxes_labels == -1)
            ...
            match_boxes_labels = paddle.where(
                neg_mask,
                paddle.full_like(match_boxes_labels, gt_class.size - 1), match_boxes_labels)
            num_pos_list.append(max(1.0, pos_mask.sum().item()))

            cls_pred_list.append(cls_logit[chosen_mask])
            cls_tar_list.append(gt_class[match_boxes_labels[chosen_mask]])
