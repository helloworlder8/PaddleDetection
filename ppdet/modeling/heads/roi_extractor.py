...
class RoIAlign(nn.Layer):
    ...
    def forward(self, body_feats, bs_rois, bs_rois_num): #bs_rois, bs_rois_num
        bs_rois = paddle.concat(bs_rois) if len(bs_rois) > 1 else bs_rois[0]
        if len(body_feats) == 1:
            rois_feat = paddle.vision.ops.roi_align(
                x=body_feats[self.start_level],
                boxes=bs_rois,
                boxes_num=bs_rois_num,
                output_size=self.resolution,
                spatial_scale=self.spatial_scale[0],
                aligned=self.aligned)
        else:
            offset = 2
            k_min = self.start_level + offset #2
            k_max = self.end_level + offset #5
            ...
            rois_dist, rois_index, rois_num_dist = distribute_fpn_proposals( #->[[685, 4],[257, 4],[68, 4],[14, 4]] [1024, 1] [[2],[2],[2],[2]]
                bs_rois, #[1024, 4]
                k_min, #2
                k_max, #5
                self.canconical_level, #4
                self.canonical_size, #224
                rois_num=bs_rois_num) #[2]
            ...
            rois_feat = paddle.gather(rois_feat_shuffle, rois_index)
