...
class CenterNetHead(nn.Layer):
    ...
    def get_loss(self, inputs, weights, head_outs):
        # 1. Heatmap (hm) loss using CTFocalLoss
        pd_heatmap = paddle.clip(head_outs['heatmap'], 1e-4, 1 - 1e-4)
        tgt_heatmap = inputs['heatmap']
        ctfocal_loss = CTFocalLoss()
        heatmap_loss = ctfocal_loss(pd_heatmap, tgt_heatmap)
        ...
        inds = paddle.concat(x=[batch_inds, loc], axis=2)#->[2, 500, 2]
        loc_inds_mask = inputs['index_mask'] #->[2, 500] 1或者0
        ...
        samp_pd_size = paddle.gather_nd(pd_size, index=inds) #->[2, 500, 4]
        size_mask = paddle.expand_as(loc_inds_mask, samp_pd_size)
        ...
        samp_pd_offset = paddle.gather_nd(pd_offset, index=inds)
        offset_mask = paddle.expand_as(loc_inds_mask, samp_pd_offset)
        ...
        if self.add_iou and 'iou' in self.loss_weight:
            ...
            pos_iou = paddle.gather_nd(iou, index=inds)
            iou_mask = paddle.expand_as(loc_inds_mask, pos_iou)
    ...
    def get_loss(self, inputs, weights, head_outs):
        ...
        # 2. Size (wh) loss: L1 loss or GIoU loss
        pd_size = paddle.transpose(head_outs['size'], perm=[0, 2, 3, 1])  # [2, 152, 272, 4]
        pd_size = paddle.reshape(pd_size, shape=[pd_size.shape[0], -1, pd_size.shape[-1]])  # [2, 41344, 4]
        samp_pd_size = paddle.gather_nd(pd_size, inds)  # [2, 500, 4]
        size_mask = paddle.cast(paddle.expand_as(loc_inds_mask, samp_pd_size), dtype=samp_pd_size.dtype)
        pos_size_mask_num = size_mask.sum()

        # Choose loss type (L1 or GIoU)
        size_loss = 0.0
        if self.size_loss == 'L1':
            tgt_size = self._get_target_size(inputs)
            size_loss = F.l1_loss(samp_pd_size * size_mask, tgt_size * size_mask, reduction='sum')
            size_loss /= (pos_size_mask_num + 1e-4)
        elif self.size_loss == 'giou':
            size_loss = self._giou_loss(samp_pd_size, size_mask, inputs)

        # 3. Offset (reg) loss: L1 loss
        pd_offset = paddle.transpose(head_outs['offset'], perm=[0, 2, 3, 1])  # [2, 152, 272, 2]
        pd_offset = paddle.reshape(pd_offset, shape=[pd_offset.shape[0], -1, pd_offset.shape[-1]])  # [2, 41344, 2]
        samp_pd_offset = paddle.gather_nd(pd_offset, inds)  # [2, 500, 2]
        offset_mask = paddle.cast(paddle.expand_as(loc_inds_mask, samp_pd_offset), dtype=samp_pd_offset.dtype)
        pos_offset_mask_num = offset_mask.sum()
        offset_loss = F.l1_loss(samp_pd_offset * offset_mask, inputs['offset'] * offset_mask, reduction='sum')
        offset_loss /= (pos_offset_mask_num + 1e-4)

        # 4. IOU loss (optional): GIoU loss
        iou_loss = 0.0
        if self.add_iou and 'iou' in self.loss_weight:
            iou = paddle.transpose(head_outs['iou'], perm=[0, 2, 3, 1])  # [2, 152, 272, C]
            iou = paddle.reshape(iou, shape=[iou.shape[0], -1, iou.shape[-1]])  # [2, 41344, C]
            pos_iou = paddle.gather_nd(iou, inds)  # [2, 500, C]
            iou_mask = paddle.cast(paddle.expand_as(loc_inds_mask, pos_iou), dtype=pos_iou.dtype)
            pos_offset_mask_num = iou_mask.sum()

            gt_bbox_xys = inputs['bbox_xyxy']
            pred_boxes = self._get_pred_boxes(samp_pd_size, gt_bbox_xys)
            giou_loss = GIoULoss(reduction='sum')
            iou_loss = giou_loss(pred_boxes * iou_mask, gt_bbox_xys * iou_mask, iou_weight=iou_mask)
            iou_loss /= (pos_offset_mask_num + 1e-4)
    ...
    def _get_target_size(self, inputs):
        """ Helper function to get target size """
        if self.regress_ltrb:
            return inputs['size']
        elif inputs['size'].shape[-1] == 2:
            return inputs['size']  # width, height
        else:
            return inputs['size'][:, :, 0:2] + inputs['size'][:, :, 2:]  # x1, y1, x2, y2 -> width, height


    def _giou_loss(self, samp_pd_size, size_mask, inputs):
        """ Helper function to calculate GIoU loss """
        tgt_bbox = inputs['bbox_xyxy']
        centers_x = (tgt_bbox[:, :, 0:1] + tgt_bbox[:, :, 2:3]) / 2.0
        centers_y = (tgt_bbox[:, :, 1:2] + tgt_bbox[:, :, 3:4]) / 2.0
        x1 = centers_x - samp_pd_size[:, :, 0:1]
        ...
        pred_boxes = paddle.concat([x1, y1, x2, y2], axis=-1)
        giou_loss = GIoULoss(reduction='sum')
        return giou_loss(pred_boxes * size_mask, tgt_bbox * size_mask, iou_weight=size_mask)
        

    def _get_pred_boxes(self, samp_pd_size, gt_bbox_xys):
        """ Helper function to get predicted bounding boxes """
        centers_x = (gt_bbox_xys[:, :, 0:1] + gt_bbox_xys[:, :, 2:3]) / 2.0
        centers_y = (gt_bbox_xys[:, :, 1:2] + gt_bbox_xys[:, :, 3:4]) / 2.0
        x1 = centers_x - samp_pd_size[:, :, 0:1]
        y1 = centers_y - samp_pd_size[:, :, 1:2]
        x2 = centers_x + samp_pd_size[:, :, 2:3]
        y2 = centers_y + samp_pd_size[:, :, 3:4]
        return paddle.concat([x1, y1, x2, y2], axis=-1)