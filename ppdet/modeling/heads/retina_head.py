...
class RetinaHead(nn.Layer):
    ...
    def get_loss(self, head_outputs, targets):
        ...
        # match_boxes_labels: -1(ignore), 0(neg) or 1(pos)
        matches_list, match_labels_list = [], []
        # assign anchors to gts, no sampling is involved
        for gt_bbox in targets['gt_bbox']:
            matches, match_boxes_labels = self.bbox_assigner(anchors, gt_bbox)
            matches_list.append(matches)
            match_labels_list.append(match_boxes_labels)
        ...
        for matches, match_boxes_labels, cls_logit, bbox_reg, gt_bbox, gt_class in \
            zip(matches_list, match_labels_list, cls_logits, bboxes_reg,
                targets['gt_bbox'], targets['gt_class']):
            pos_mask = (match_boxes_labels == 1)
            neg_mask = (match_boxes_labels == 0)
