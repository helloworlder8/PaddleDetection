...
class DETRHead(nn.Layer):
    ...
    def forward(self, out_transformer, body_feats, inputs=None):
        ...
        outputs_logit = self.score_head(feats) #[6, 2, 100, 256] ->[6, 2, 100, 5]
        outputs_bbox = F.sigmoid(self.bbox_head(feats)) #->[6, 2, 100, 4]
        outputs_seg = None
        if self.with_mask_head:
        ...
        if self.training:
            ...
            gt_mask = self.get_gt_mask_from_polygons( #分割
                inputs['gt_poly'],
