...
class CTFocalLoss(object):
    ...
    def __call__(self, pred, target):
        ...
        """
        # fg_map = paddle.cast(target == 1, 'float32') #满足条件设置成1
        ...
        # 创建前景和背景掩码
        fg_mask = paddle.cast(target == 1, 'float32')
        fg_mask.stop_gradient = True

        bg_mask = paddle.cast(target < 1, 'float32')
        bg_mask.stop_gradient = True

        # 计算负样本的权重
        bg_weights = paddle.pow(1 - target, 4)

        # 计算正样本损失（前景损失）
        posit_loss = -paddle.log(pred) * paddle.pow(1 - pred, self.gamma) * fg_mask

        # 计算负样本损失（背景损失）
        neg_loss = -paddle.log(1 - pred) * paddle.pow(pred, self.gamma) * bg_weights * bg_mask

        # 计算损失的总和
        posit_loss_sum = paddle.sum(posit_loss)
        neg_loss_sum = paddle.sum(neg_loss)

        # 计算前景样本数
        fg_num = paddle.sum(fg_mask)

        # 计算聚焦损失
        focal_loss = (posit_loss_sum + neg_loss_sum) / (
            fg_num + paddle.cast(fg_num == 0, 'float32')
        )
