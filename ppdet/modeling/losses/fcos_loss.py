...
class FCOSLossMILC(FCOSLoss):
    ...
    def forward(self, cls_logits, bboxes_reg, centerness, tag_labels,
        ...
        with paddle.no_grad():
            ...
            pos_pred = bboxes_reg_flatten[pos_inds]
            pos_target = tag_bboxes_flatten[pos_inds]
            bbox_iou = self._bbox_overlap_align(pos_pred, pos_target)
        # pos labels
        pos_labels = tag_labels_flatten[pos_inds].squeeze(1)
        cls_target = paddle.zeros(cls_logits_flatten.shape)
        cls_target[pos_inds, pos_labels - 1] = bbox_iou
        ...
        # 3. iou loss
        pos_iou_pred = paddle.squeeze(centerness_flatten, axis=-1)[pos_inds]
...
class FCOSLossCR(FCOSLossMILC):
    ...
    def hard_neg_mining(self,
        ...
        # decode pos bbox
        pos_cls = max_vals[pos_inds]
        pos_loc = loc_ltrb[pos_inds].reshape([-1, 4])
        pos_strides = strides[pos_inds]
        pos_points = points[pos_inds].reshape([-1, 2])
        pos_loc = pos_loc * pos_strides
        pos_bbox = self.decode_bbox(pos_loc, pos_points)
        pos_scales = scales[pos_inds]
    ...
    def get_targets_per_img(self, tea_cls, tea_loc, tea_iou, stu_cls, stu_loc,
        ...
        if num_candidate > 0:
            ...
            pos_thresh = (candidate_score_mean + candidate_score_std).clip(
                max=0.4)
            # select pos
            pos_inds = paddle.nonzero(max_vals >= pos_thresh).squeeze(axis=-1)
        ...
        if (num_pos == 0):
            num_pos = 10
            _, pos_inds = paddle.topk(max_vals, k=num_pos)
        cls_mask[pos_inds] = 1.

        ### Consistency Regularization Training targets
        # cls targets
        pos_class_ind = class_ind[pos_inds]
        cls_targets = paddle.zeros_like(tea_cls)
        cls_targets[pos_inds, pos_class_ind] = tea_cls_scores[pos_inds,
                                                             pos_class_ind]
        ...
        loc_targets[pos_inds] = tea_loc[pos_inds]
        # iou targets
        iou_targets = paddle.zeros(
            shape=[tea_iou.shape[0]], dtype=tea_iou.dtype)
        iou_targets[pos_inds] = F.sigmoid(
            paddle.squeeze(
                tea_iou, axis=-1)[pos_inds])
        ...
        if (num_hard_neg > 0) and self.hard_neg_mining_flag:
            results = self.hard_neg_mining(tea_cls, tea_loc, tea_iou, pos_inds,
