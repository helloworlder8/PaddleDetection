...
class RPNTargetAssign(object):
    ...
    Args:
        num_samples_per_img (int): Total number of RPN samples per image. 
            default 256
    ...
    def __init__(self,
        ...
        self.num_samples_per_img = num_samples_per_img
    ...
    def __call__(self, inputs, anchors):
        ...
        """
        gt_boxes = inputs['gt_bbox'] #[[4, 4],[7, 4]]
        is_crowd = inputs.get('is_crowd', None)#[[4, 1],[7, 1]]
        batch_size = len(gt_boxes)
        bs_sample_boxes_labels, bs_tgt_rpn_boxes, bs_tgt_rpn_delta = rpn_anchor_target(
            anchors,  #[257796, 4]
            gt_boxes, #[[4, 4],[7, 4]]
            self.num_samples_per_img, #256
            self.positive_overlap, #0.7
            self.negative_overlap, #0.3
            self.fg_fraction, #0.5
            self.use_random, #true
            batch_size, #2
            self.ignore_thresh, #-1
            is_crowd, #[[4, 1],[7, 1]]
            assign_on_cpu=self.assign_on_cpu)
        bs_num_samples = self.num_samples_per_img * batch_size
# bs_sample_boxes_labels, bs_tgt_rpn_boxes, bs_tgt_rpn_delta
        return bs_sample_boxes_labels, bs_tgt_rpn_boxes, bs_tgt_rpn_delta, bs_num_samples
...
class BBoxAssigner(object):
    ...
    Args:
        num_samples_per_img (int): Total number of RoIs per image. 
            default 512 
    ...
    def __call__(self,
        ...
        # rois, gt_labels, gt_bboxes, tgt_gt_labels
        # new_rois_num
        outs = generate_proposal_target(
            rpn_rois, gt_classes, gt_boxes, self.num_samples_per_img,
        ...
        proposals_box = outs[0]
        rois_num = outs[-1]
        # gt_labels, gt_bboxes, tgt_gt_labels
        targets = outs[1:4]
        return proposals_box, rois_num, targets
...
class MaskAssigner(object):
    ...
    def __call__(self, rois, gt_labels, tgt_gt_labels, inputs):
        gt_segms = inputs['gt_poly']

        outs = generate_mask_target(gt_segms, rois, gt_labels, tgt_gt_labels,
