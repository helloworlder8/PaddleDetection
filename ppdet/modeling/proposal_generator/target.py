...
def sample_bbox(match_which_boxes, #[1004]
    ...
    num_gt = tgt_classes.shape[0]
    if num_gt == 0:
        # No truth, assign everything to background
        tgt_classes = paddle.ones(match_which_boxes.shape, dtype='int32') * num_classes
        #return match_which_boxes, match_boxes_labels + num_classes
    else:
        tgt_classes = paddle.gather(tgt_classes, match_which_boxes) #->[1004]
