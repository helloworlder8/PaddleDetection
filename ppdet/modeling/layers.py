...
class ConvNormLayer(nn.Layer):
    ...
    def forward(self, inputs):
        out = self.conv(inputs) #[2, 3, 608, 1088]->[2, 16, 608, 1088]
        if self.norm is not None:
            out = self.norm(out) #->[2, 16, 608, 1088]
        return out
...
#preds, bs_rois_and_num, im_shape, scale_factor
    def __call__(self, preds, bs_rois_and_num, im_shape, scale_factor):
        pd_bbox_deltas = preds[0]
        pd_bbox_class = preds[1]
        bs_rois = bs_rois_and_num[0]
        bs_rois_num = bs_rois_and_num[1]

        if self.export_onnx:
            onnx_rois_num_per_img = bs_rois_num[0]
            expand_origin_img_shape = paddle.expand(im_shape[0, :],
                                         [onnx_rois_num_per_img, 2])

        else:
            origin_shape_list = []
            if isinstance(bs_rois, list):
                batch_size = len(bs_rois)
            else:
                batch_size = paddle.slice(im_shape.shape, [0], [0], [1])

            # pd_bbox_deltas.shape: [N, C*4]
            for idx in range(batch_size):
                rois_num_per_img = bs_rois_num[idx]
                expand_im_shape = paddle.expand(im_shape[idx, :],
                                                [rois_num_per_img, 2])
                origin_shape_list.append(expand_im_shape)

            expand_origin_img_shape = paddle.concat(origin_shape_list)

        # pd_bbox_deltas.shape: [N, C*4]
        # C=num_classes in faster/mask rcnn(bbox_head), C=1 in cascade rcnn(cascade_head)
        bs_rois = paddle.concat(bs_rois)
        bs_rois = delta2bbox(pd_bbox_deltas, bs_rois, self.prior_box_var)
        pd_bbox_class_without_bg = pd_bbox_class[:, :-1] #除去背景类

        # bs_rois.shape: [N, C, 4]
        # bs_rois.shape[1] must be equal to scores.shape[1]
        total_num = bs_rois.shape[0]
        rois_dim = bs_rois.shape[-1]
        expand_bs_rois = paddle.expand(bs_rois, [total_num, self.num_classes, rois_dim])

        expand_origin_h = paddle.unsqueeze(expand_origin_img_shape[:, 0], axis=1)
        expand_origin_w = paddle.unsqueeze(expand_origin_img_shape[:, 1], axis=1)
        zeros = paddle.zeros_like(expand_origin_h)
        x1 = paddle.maximum(paddle.minimum(expand_bs_rois[:, :, 0], expand_origin_w), zeros)
        y1 = paddle.maximum(paddle.minimum(expand_bs_rois[:, :, 1], expand_origin_h), zeros)
        x2 = paddle.maximum(paddle.minimum(expand_bs_rois[:, :, 2], expand_origin_w), zeros)
        y2 = paddle.maximum(paddle.minimum(expand_bs_rois[:, :, 3], expand_origin_h), zeros)
        expand_bs_rois = paddle.stack([x1, y1, x2, y2], axis=-1)
        expand_bs_rois_and_num = (expand_bs_rois, bs_rois_num)
        return expand_bs_rois_and_num, pd_bbox_class_without_bg #expand_bs_rois_and_num
...
class MultiClassNMS(object):
    def __init__(self,
        ...
        self.trt = trt
        self.cpu = cpu
# expand_bs_rois_and_num, pd_bbox_class_without_bg, self.num_classes
    def __call__(self, bboxes, score, background_label=-1):
        ...
        if isinstance(bboxes, tuple):
            ...
            kwargs.update({'rois_num': paddle.to_tensor(bbox_num)})
        if background_label > -1: #paddle.to_tensor(rois_num_np)
        ...
        if self.trt and (int(paddle.version.major) == 0 or
            ...
            bs_rois, bbox_num, _ = ops.multiclass_nms(bboxes, score, **kwargs)
            bs_rois = bs_rois.reshape([1, -1, 6])
            idx = paddle.nonzero(bs_rois[..., 0] != -1)
            bs_rois = paddle.gather_nd(bs_rois, idx)
            return bs_rois, bbox_num, None
        else:
            if self.cpu:
...
class MatrixNMS(object):
    ...
    def __call__(self, bs_rois, score, *args):
...
class YOLOBox(object):
    ...
    def __call__(self,
        ...
        boxes_list = []
        scores_list = []
        expand_origin_img_shape = im_shape / scale_factor
        expand_origin_img_shape = paddle.cast(expand_origin_img_shape, 'int32')
        for i, head_out in enumerate(yolo_head_out):
            boxes, scores = paddle.vision.ops.yolo_box(
                head_out,
                expand_origin_img_shape,
                anchors[i],
                self.num_classes,
                self.conf_thresh,
...
class TTFBox(object):
    ...
    def _decode(self, hm, wh, im_shape, scale_factor):
        ...
        scores = results[:, 1]
        valid_inds = paddle.nonzero(scores > self.score_thresh)
        results = paddle.gather(results, valid_inds)
