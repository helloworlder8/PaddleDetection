...
def bbox2delta(src_boxes, tgt_boxes, weights=[1.0, 1.0, 1.0, 1.0]):
    ...
    """
    src_w = src_boxes[:, 2] - src_boxes[:, 0] #->[257796]
    src_h = src_boxes[:, 3] - src_boxes[:, 1] #->[257796]
    src_ctr_x = src_boxes[:, 0] + 0.5 * src_w #->[257796]
    src_ctr_y = src_boxes[:, 1] + 0.5 * src_h #->[257796]
    ...
    wx, wy, ww, wh = weights
    dx = wx * (tgt_ctr_x - src_ctr_x) / src_w #中心点宽差
    dy = wy * (tgt_ctr_y - src_ctr_y) / src_h #中心点高差
    dw = ww * paddle.log(tgt_w / src_w) #宽度偏差
    dh = wh * paddle.log(tgt_h / src_h) #高度偏差
...
def bbox_overlaps(boxes1, boxes2):
    ...
    area1 = bbox_area(boxes1) #->[4]
    area2 = bbox_area(boxes2) #->[257796]
    
    #交集区域的最小值 [4, 257796, 2]
    xy_max = paddle.minimum(paddle.unsqueeze(boxes1, 1)[:, :, 2:], boxes2[:, 2:])
    #交集区域的最大值 [4, 257796, 2]
    xy_min = paddle.maximum(paddle.unsqueeze(boxes1, 1)[:, :, :2], boxes2[:, :2])
    width_height = xy_max - xy_min #[4, 257796, 2]
    width_height = width_height.clip(min=0) #[4, 257796, 2]
    inter = width_height.prod(axis=2) #在第二个维度做乘积

    overlaps = paddle.where(inter > 0, inter /(paddle.unsqueeze(area1, 1) + area2 - inter),paddle.zeros_like(inter))
    return overlaps
