...
class ResNet(nn.Layer):
    ...
    def forward(self, inputs):
        x = inputs['image'] #[2, 3, 768, 1344]
        conv1 = self.conv1(x) #->[2, 64, 384, 672]
        x = F.max_pool2d(conv1, kernel_size=3, stride=2, padding=1) #[2, 64, 192, 336]
        outs = []
        for idx, stage in enumerate(self.res_layers):
            x = stage(x)
            if idx in self.return_idx:
                outs.append(x)
        return outs #->[2, 64, 192, 336] [2, 128, 96, 168] [2, 256, 48, 84] [2, 512, 24, 42]
