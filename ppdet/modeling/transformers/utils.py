...
def get_contrastive_denoising_training_group(gt_meta,#gt_meta
                                             num_classes,
                                             num_queries,
                                             class_embed,
                                             num_denoising=100,
    ...
    if num_denoising <= 0:
        return None, None, None, None
    num_gts = [len(t) for t in gt_meta["gt_class"]] # [2, 1]
    max_gt_num = max(num_gts) #2
    if max_gt_num == 0:
        return None, None, None, None

    num_group = num_denoising // max_gt_num # 50
    num_group = 1 if num_group == 0 else num_group
    # pad gt to max_num of a batch
    bs = len(gt_meta["gt_class"]) #2
    input_query_class = paddle.full(
        [bs, max_gt_num], num_classes, dtype='int32')
    input_query_bbox = paddle.zeros([bs, max_gt_num, 4])
    pad_gt_mask = paddle.zeros([bs, max_gt_num])
    for i in range(bs):
        num_gt = num_gts[i]
        if num_gt > 0:
            input_query_class[i, :num_gt] = gt_meta["gt_class"][i].squeeze(-1) #批 最大
            input_query_bbox[i, :num_gt] = gt_meta["gt_bbox"][i] #[2, 2, 4] 批 最大 bbox
            pad_gt_mask[i, :num_gt] = 1
