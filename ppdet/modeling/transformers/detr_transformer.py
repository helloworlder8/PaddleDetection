...
class DETRTransformer(nn.Layer):
    ...
    def forward(self, src, src_mask=None, *args, **kwargs):
        ...
        src_proj = self.input_proj(src[-1]) #[2, 2048, 16, 20]->[2, 256, 16, 20]
        bs, c, h, w = src_proj.shape #2 256 16 20
        # flatten [B, C, H, W] to [B, HxW, C]
        src_flatten = src_proj.flatten(2).transpose([0, 2, 1]) #[2, 320, 256]
        if src_mask is not None:
            src_mask = F.interpolate(src_mask.unsqueeze(0), size=(h, w))[0]
        else:
            src_mask = paddle.ones([bs, h, w])
        pos_embed = self.position_embedding(src_mask).flatten(1, 2) #->[2, 320, 256]

        if self.training:
            src_mask = self._convert_attention_mask(src_mask) #->[2, 16, 20]
            src_mask = src_mask.reshape([bs, 1, 1, h * w])
        else:
            src_mask = None

        memory = self.encoder( #->[2, 320, 256]
            src_flatten, src_mask=src_mask, pos_embed=pos_embed)

        query_pos_embed = self.query_pos_embed.weight.unsqueeze(0).tile(
            [bs, 1, 1]) #->[2, 100, 256]
        tgt = paddle.zeros_like(query_pos_embed) #->[2, 100, 256]
        output = self.decoder( #->[6, 2, 100, 256]
            tgt,
            memory,
