...
class RTDETRTransformer(nn.Layer):
    ...
    def _reset_parameters(self):
        ...
        # init encoder output anchors and valid_masks
        if self.eval_size:
            self.anchors, self.valid_masks = self._generate_anchors()
    ...
    def _get_encoder_input(self, feats):
        ...
        # get encoder inputs
        feat_flatten = [] #->[1, 4624, 256] [1, 1156, 256] [1, 289, 256]
        spatial_shapes = [] #->[[68, 68], [34, 34], [17, 17]]
        level_start_index = [0, ] #->[0, 4624, 5780, 6069]
        for i, feat in enumerate(proj_feats):
            _, _, h, w = feat.shape
            # [b, c, h, w] -> [b, h*w, c]
            feat_flatten.append(feat.flatten(2).transpose([0, 2, 1])) #[[1, 4624, 256]]
            # [num_levels, 2]
        ...
        # [b, l, c]
        feat_flatten = paddle.concat(feat_flatten, 1) #->[1, 6069, 256]
        level_start_index.pop() #[0, 4624, 5780]
        return (feat_flatten, spatial_shapes, level_start_index)

    def forward(self, feats, pad_mask=None, gt_meta=None, is_teacher=False):
        # input projection and embedding
        (feat_flatten, spatial_shapes, level_start_index) = self._get_encoder_input(feats)

        # prepare denoising training
        if self.training:
            denoising_class, denoising_bbox_unact, attn_mask, dn_meta = get_contrastive_denoising_training_group(gt_meta,
                                            self.num_classes,
                                            self.num_queries,
        ...
        target, init_ref_points_unact, enc_topk_bboxes, enc_topk_logits = \
            ...
            feat_flatten, spatial_shapes, denoising_class, denoising_bbox_unact,is_teacher)
    ...
    def _get_decoder_input(self,
        ...
        if self.training or self.eval_size is None or is_teacher:
            anchors, valid_masks = self._generate_anchors(spatial_shapes)
        else:
            anchors, valid_masks = self.anchors, self.valid_masks
        memory = paddle.where(valid_masks, memory, paddle.to_tensor(0.))
