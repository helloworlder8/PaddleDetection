...
class HybridEncoder(nn.Layer):
    ...
    def forward(self, feats, for_mot=False, is_teacher=False):
        ...
        proj_feats = [self.input_proj[i](feat) for i, feat in enumerate(feats)] # [1, 256, 68, 68] [1, 256, 34, 34] [1, 256, 17, 17]
        # encoder
        if self.num_encoder_layers > 0:
            for i, enc_ind in enumerate(self.use_encoder_idx):
                h, w = proj_feats[enc_ind].shape[2:] #最上面的一层
                # flatten [B, C, H, W] to [B, HxW, C]
                src_flatten = proj_feats[enc_ind].flatten(2).transpose(
                    [0, 2, 1]) #-》[1, 256, 289] -》[1, 289, 256] 批 宽高 维
                if self.training or self.eval_size is None or is_teacher:
                    pos_embed = self.build_2d_sincos_position_embedding(
                        w, h, self.hidden_dim, self.pe_temperature) #[1, 289, 256]
                else:
