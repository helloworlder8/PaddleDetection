...
class UniformAssigner(nn.Layer):
    ...
    def forward(self, bbox_pred, anchor, gt_bboxes, gt_labels=None):
        ...
        match_boxes_labels = paddle.full([num_bboxes], -1, dtype=paddle.int32)
        ...
        #match_boxes_labels: -2(ignore), -1(neg) or >=0(pos_inds)
        match_boxes_labels = paddle.where(neg_ignore,
                                    paddle.full_like(match_boxes_labels, -2),
                                    match_boxes_labels)
        ...
        match_boxes_labels[pos_places] = pos_inds
        match_boxes_labels.stop_gradient = True
        pos_keep = ~pos_ignore
        ...
        return match_boxes_labels, pos_bbox_pred, pos_bbox_tar
