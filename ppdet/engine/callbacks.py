    ...
    def on_step_end(self, status):
        if dist.get_world_size() < 2 or dist.get_rank() == 0:
            ...
            if mode == 'train':
                ...
                epoches = self.model.cfg_dict.epoch
                batch_size = self.model.cfg_dict['{}Reader'.format(mode.capitalize(
                ))]['batch_size']

                logs = training_staus.log()
                space_fmt = ':' + str(len(str(steps_per_epoch))) + 'd'
                if step_id % self.model.cfg_dict.log_iter == 0:
                    ...
                    fmt = ' '.join([
                        '\n\033[1;34mEpoch: [{}]\033[0m',  # 加粗蓝色
                        '\033[1;34m[{' + space_fmt + '}/{}]\033[0m',  # 加粗蓝色
                        'learning_rate: {lr:.6f}',
                        '{meters}',
                        'eta: {eta}',
                        'batch_cost: {btime}',
...
class Checkpointer(Callback):
    def __init__(self, model):
        ...
        self.best_ap = -1000.
        self.save_dir = self.model.cfg_dict.save_dir
    ...
    def on_epoch_end(self, status):
        ...
        save_name = None
        if dist.get_world_size() < 2 or dist.get_rank() == 0:
            end_epoch = self.model.cfg_dict.epoch
            save_name = str(epoch_id) if epoch_id != end_epoch - 1 else "model_final"
            if mode == 'train':
                end_epoch = self.model.cfg_dict.epoch
                if (
                        epoch_id + 1
                ) % self.model.cfg_dict.snapshot_epoch == 0 or epoch_id == end_epoch - 1:
                    save_name = str(
                ...
                for metric in self.model._metrics:
                    ...
                    key = self.model.cfg_dict.get('target_metrics', key)
...
class VisualDLWriter(Callback):
    ...
    def __init__(self, model):
        ...
        self.vdl_writer = LogWriter(
            model.cfg_dict.get('vdl_log_dir', 'vdl_log_dir/scalar'))
...
class WandbCallback(Callback):
    def __init__(self, model):
        ...
        self.wandb_params = model.cfg_dict.get('wandb', None)
        self.save_dir = self.model.cfg_dict.save_dir
        if self.wandb_params is None:
            self.wandb_params = {}
        for k, v in model.cfg_dict.items():
        ...
        if dist.get_world_size() < 2 or dist.get_rank() == 0:
            _ = self.run
            self.run.config.update(self.model.cfg_dict)
...
class SemiCheckpointer(Checkpointer):
    def __init__(self, model):
        ...
        cfg = self.model.cfg_dict
        self.best_ap = 0.
        self.save_dir = os.path.join(self.model.cfg_dict.save_dir,
                                     self.model.cfg_dict.filename)
